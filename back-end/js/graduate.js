function c(){
	...
	$(".del").click(function(){
		if(confirm("确定要删除数据吗")){
			var url = "/member/memberId/"+ $(this).parent("td").siblings().eq(0).text();
			console.log(url);
			$.ajax({
				type : "POST",
				url : url,
		...
		success : function(data){
			...
			p.then(function(){ 
				...
				for(var i=1;i<=pageNum;i++){
					pageStr1+='<li class="graduatePage_li">'+ i +'</li>';
				}
				pageStr2 = '<a class="graduatePage_prev graduatePage_change" id="graduatePage_prev">上一页</a><ul class="graduatePage_ul">'+
							pageStr1+
							'</ul>'+
							'<a class="graduatePage_next graduatePage_change" id="graduatePage_next">下一页</a>';
				$(".graduatePage").html(pageStr2);

				//按钮高亮显示
				$(".graduatePage_li").eq(nowPage-1).css({
				...
				// 分页数字按钮点击
				$(".graduatePage li").click(function(){
					nowPage = $(this).text();
					$.ajax({
						type : "GET",
						url : "/member/listOldMembers/"+nowPage,
						success : function(data){
							if (data.success == true) {
								var oData = data.data.listMembers;
								var len = oData.length;
				...
				// 上一页点击
				$("#graduatePage_prev").click(function(){
					if (nowPage == 1) {
						alert("已经是第一页了");
					}else{
					nowPage = nowPage - 1;
				...
				//下一页点击
				$("#graduatePage_next").click(function(){
					if (nowPage == pageNum) {
						alert("已经是最后一页了");
					}else{
					nowPage = nowPage + 1;
...
$("#W_searchBtn").click(function(){
	var nowPage = 1;
	var url = "/member/memberId/"+$("#W_search").val()+"/"+nowPage;
	$.ajax({
		type : "GET",
		url : url,
		success : function(data){
			if (data.success == true) {
				if(data.data == "not found."){
					...
					pageStr = '<a class="graduatePage_prev graduatePage_change" id="graduatePage_prev_1">上一页</a>'+
							  '<ul class="graduatePage_ul">'+
							  	pageStr_1+
							  '</ul>'+
							  '<a class="graduatePage_next graduatePage_change" id="graduatePage_next_1">下一页</a>';
					$("graduatePage").html(pageStr);
